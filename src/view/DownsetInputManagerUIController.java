...
import model.RelationsGraphGenerator;
...
public class DownsetInputManagerUIController implements Initializable {
    ...
    @FXML
    private Label relationGraphLabel;
    ...
    @FXML
    private Label invalidInputMessageLabel;
    ...
    private void updateElementsComboBox(){
        if(!checkInput(setElementsTextField.getText())){
            invalidInputMessageLabel.setText("Invalid input. Choose a number n ∈ N | n > 0");
        }
        else{
            invalidInputMessageLabel.setText("");
            List<String> list = new ArrayList<>();
    ...
    private void handleGenerateGraphButtonAction(ActionEvent event) throws IOException{
        if(!relations.isEmpty()){
            ...
            RelationsGraphGenerator generator = new RelationsGraphGenerator(relations);
            generator.generateGraph();


            //display png image
            imageView = new ImageView();
            File f = new File("relationsgraph.png");
            ...
            generateDownsetButton.setVisible(true);
        }
    }
    
    @FXML
    private void handleGenerateDownsetGraphButtonAction(ActionEvent event) throws IOException{
        resetButton.setVisible(true);
        
        DownsetGraphGenerator generator2 = new DownsetGraphGenerator(setElements, relations);
        generator2.generateGraph();
        
        //open new window and display downset graph
        Stage stage = new Stage();
        Parent root = FXMLLoader.load(getClass().getResource("/view/DownsetVisualizatorUI.fxml")); //edit string to test window
    ...
    private void handleResetButtonAction() throws IOException{
        ...
        invalidInputMessageLabel.setText("");
        generateDownsetButton.setVisible(false);
